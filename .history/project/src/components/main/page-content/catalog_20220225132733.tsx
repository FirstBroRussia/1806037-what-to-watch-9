import {FILM_CARD_COUNT, CustomFilmData, CustomFilmDataType} from '../../utils/utils';
import {FilmCardForCatalog} from './film-card-for-catalog';


function CreateJSXArrayFilmsCatalog(filmData: CustomFilmDataType): any {
  const JSXArray = [];
  for (let index = 1; index <= FILM_CARD_COUNT; index++) {
    JSXArray.push(<FilmCardForCatalog props={filmData} />);
  }
  return JSXArray;
}

console.log(CreateJSXArrayFilmsCatalog(FilmCardForCatalog));
